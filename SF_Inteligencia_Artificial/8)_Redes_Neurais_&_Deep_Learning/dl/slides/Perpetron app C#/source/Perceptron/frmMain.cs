    ...
    public partial class frmMain : Form
    {
        ...
        private void btnLearn_Click(object sender, EventArgs e)
        {
            ...
            while (error && iterations < maxIterations)
            {
                error = false;
                epoch++;
                lbresumo.Items.Add(Convert.ToString(" Epoch: " + Convert.ToString(epoch)));
                iteracao = 0;
                for (i = 0; i <= samples.Count - 1; i++)
                {
                    ...
                    lbresumo.Items.Add(Convert.ToString("Equação: " + ((w1 * x1) + (w2 * x2) - w0) + "x1: " +  x1 + " x2: " +  " x0:" +  x0 + " w0: " + w0 + " w1: " +  " w2: " +  w2 + " + Erro:" + error + " Y: " + y +  " Iteração" + iteracao));
        ...
        {
            lblLearningRate.Text = "Taxa de Aprendizado: " + (double)trackLearningRate.Value/1000;
        }
        ...
        private void pnlCanvas_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                lbblue.Items.Add(Convert.ToString(posX) + "  " +  Convert.ToString(posY));
            }
            else
            {
                pen = new Pen(Color.Red);
                sample = new Sample(posX, posY, -1);
                lbred.Items.Add(Convert.ToString(posX) + "  " + Convert.ToString(posY));
            }
            samples.Add(sample);
