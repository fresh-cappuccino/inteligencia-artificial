...
ag = inicializaPopulacao(algoritmoGenetico = ag, espacos = espacos, valores = valores, limite = limite)
...
for (individuosGerados in 1:(ag@tamanhoPopulacao / 2)) {
  ...
  novaPopulacao = c(novaPopulacao, mutacao(individuo = filhos[[1]], taxaMutacao = probabilidadeMutacao))
  novaPopulacao = c(novaPopulacao, mutacao(individuo = filhos[[2]], taxaMutacao = probabilidadeMutacao))
}
...
cat("Melhor solução: ", ag@melhorSolucao@cromossomo, " Valor: ", ag@melhorSolucao@notaAvaliacao)
